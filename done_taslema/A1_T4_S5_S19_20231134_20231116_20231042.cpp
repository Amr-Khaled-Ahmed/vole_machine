        ...
        if (extension == ".txt") {
            ...
            while (getline(ProgramFile, line)){
                ...
                if (ProgramCounter >= 256) {
                    cerr << "Warning: Program size exceeds memory limit.\a" << endl ;
                }
                ...
                if (Ins.getOpCode_from_ins() == '1') { // LOAD the register R with the bit pattern found in the memory cell whose address is XY.
                    ...
                    if (XY == 0) {
                        ...
                        memory.set_memory_cells(ProgramCounter, 00);
                    ...
                    */
                    ...
                    cout << "\n the index " << destReg << "the result is " << result << endl << "the hxa for the result " << hex << result << endl;
...
*/
                    ...
                    // Debug output to check values before the jump condition
                    cout << "Target Value (TV): " << targetValue << endl;
                    cout << "Compare Value (CV): " << compareValue << endl;

                    // Check if the target value equals the value in register 0
                    if (targetValue == compareValue) {
                        ProgramCounter = XY; // Set the program counter to the target memory address
                        cout << "Jump condition met. Program counter set to: " << hex << ProgramCounter << endl;
                    }
                    else {
                        cout << "Jump condition not met. Continuing execution." << endl;
                    }
                }
